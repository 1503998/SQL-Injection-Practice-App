...
**********************************************/
	...
	if (!file_exists('./includes/_heroes/'.$pdHero.'.html')) include_once('./includes/get_PD_Hero.php');
	   else {
	        echo file_get_contents("./includes/_heroes/$pdHero.html"); 
	        }
	$pageTitle = "$hero";
	}
	else
  ...
  {$sort = 'asc'; $sortdb = 'DESC';} else {$sort = 'desc'; $sortdb = 'ASC';}
	...
	<table class='tableA'>
	   <tr>
	       <th><div align='center'><?=$lang["hero_player_history"]?> <?=$hero?></div></th>
	   </tr>
	</table>
	</div><?php
 ...
 <tr>
  ...
  <tr class='row'>
	...
	<td width='220'><a title='<?=$list["gamename"]?>' href='game.php?gameid=<?=$gameid?>'><?=$gamename?></a></td>
	<td width='56'><?=$type?></td>
