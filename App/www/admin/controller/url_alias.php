...
class ControllerUrlAlias extends Controller {
	...
	function initialize() {//$this->initialize();  Required in each function 
		if(!isset($this->dataLink)){
			...
			$this->generate_seo =& $this->locator->get('generateseo');
			...
			$this->modelAdminAlias = $this->model->get('model_admin_urlalias');
	...
	function getList() {
		...
		$results = $this->modelAdminAlias->get_page($sql);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->ssl('url_alias', 'delete', array('url_alias_id' => $result['url_alias_id']))
      		);
      		$cell[] = array(
        		'action' => $action,
        		'align'  => 'right'
	...
	function getForm() {
		...
		if ($this->request->get('url_alias_id')) {
			...
			$view->set('delete', $this->url->ssl('url_alias', 'delete', array('url_alias_id' => $this->request->get('url_alias_id'))));
		}
