    	...
    	if ($config->get('latest_status')) {
			...
			$product_data = array();
    	  	...
    	  	foreach ($results as $result) {
				if ($config->get('latest_columns') > 3){
    	  		...
    	  		$product_data[] = array(
					...
					'thumb' => $image->resize($result['filename'], $config->get('latest_image_width'), $config->get('latest_image_height')),
			...
			$view->set('addtocart',$config->get('latest_addtocart')); // New	Shared		
			$view->set('head_def',$head_def);    // New head_def
			$view->set('columns', $config->get('latest_columns')); // New Shared
			$view->set('text_enlarge', $language->get('text_enlarge'));// New Shared
			$view->set('this_controller', 'latest'); // New Shared
