	...
	function fetch() {
    	...
    	if ($config->get('popular_status')) {
            ...
            if ($config->get('popular_limit') == '0') {
				...
				$limit = (int)$config->get('popular_limit');   //new return number to display
            }
    	  	...
    	  	foreach ($results as $result) {
				if ($result['alt_description']){
    	  		...
    	  		$product_data[] = array(
					...
					'thumb' => $image->resize($result['filename'],$config->get('popular_image_width'), $config->get('popular_image_height')),
			...
			$view->set('addtocart',$config->get('popular_addtocart'));// New Shared
			$view->set('text_enlarge', $language->get('text_enlarge'));// New Shared
			$view->set('this_controller', 'popular'); // New Shared
