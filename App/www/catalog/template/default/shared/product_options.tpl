 ...
 <div class="product_options" id="<?php echo $this_controller.'_options_'.$product['product_id']; ?>" >
  ...
  <?php foreach ($product['options'] as $key => $option) { ?>
   <tr>	  
    ...
    <td><select name="option<?php echo '_' . $product['product_id'];?>[<?php echo $key+1; ?>]" onchange="UpdateTotal(<?php echo $decimal_place . ",".$product['product_id'] . ",'" . $this_controller."'";?>)">		
    <?php foreach ($option['value'] as $price_key => $option_value) { ?>
      ...
      <?php echo $option_value['prefix']; ?><?php echo $option_value['price']; ?>
	   <?php if($price_key == '0'){
	    if ($option_value['prefix'] == '+'){
         $product_total = $product_total + number_format((float)str_replace($symbols,'',str_replace(',','',$option_value['price'])),$decimal_place,'.','');
	 ...
	 <?php $options_price[] = array(
        'price' => $option_value['prefix']."#".number_format((float)str_replace($symbols,'',str_replace(',','',$option_value['price'])),$decimal_place,'.',''),
...
<?php if ($product['options']) { ?>	
 ...
 <?php echo $price_with_options;
  $product_total = number_format($product_total,$decimal_place,'.',''); ?>
