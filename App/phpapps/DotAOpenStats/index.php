...
**********************************************/
  ...
  <TABLE>
    <TR>
	   <TD style='height:24px;'> <?=$lang['welcome_title']?></TD>
	</TR>
    <TR>
	   <TD>DotA OpenStats is Php/MySql based web statistic and CMS site for DotA Games</TD>
	</TR>
	<TR>
	   <TD><a href='https://sourceforge.net/projects/dotaopenstats/'>Download Dota OpenStats</a></TD>
    </TR>
  </TABLE>
  ...
  <?php
	...
	?>
	 ...
	 <tr>
	    <th class='padLeft'><p class='alignleft'><?=$title?></p> <p class='alignright'><?php echo $lang['posted']." ".$date; ?> <a name='<?=$row["news_id"]?>' href='index.php?id=<?=$row["news_id"]?>#<?=$row["news_id"]?>'>link</a>&nbsp;</p></th>
	 </tr>
	 <tr>
	...
	if (isset($_GET['page'])) {$mypage = '?page=';}
	 ...
	 <table class='tableNews'>
	   <tr>
	      <th class='padLeft'><p class='alignleft'><?=$title?></p> <p class='alignright'><? echo $lang['posted']." ".$date;?> <a name='<?=$row["news_id"]?>' href='index.php?page=<?=$currentpage?>&id=<?=$row["news_id"]?>#<?=$row["news_id"]?>'>link</a>&nbsp;</p>
		  </th>
	 </tr>
	 <tr>
	      <td class='NewsText'><?=$text?></td>
