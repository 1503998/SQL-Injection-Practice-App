...
class PaymentAuthNetAim extends Payment {
  	...
  	function getMethod() {
		if ($this->config->get('authnetaim_status')) {
      		if (!$this->config->get('authnetaim_geo_zone_id')) {
        		$status = true;
      		} elseif ($this->database->getRows("select * from zone_to_geo_zone where geo_zone_id = '" . (int)$this->config->get('authnetaim_geo_zone_id') . "' and country_id = '" . (int)$this->address->getCountryId($this->session->get('payment_address_id')) . "' and (zone_id = '" . (int)$this->address->getZoneId($this->session->get('payment_address_id')) . "' or zone_id = '0')")) {
      		  	$status = true;
      		} else {
     	  		$status = false;
			}	
    ...
    private function validateForm() {
        ...
        $this->errors = array();
        $ccNumber = ereg_replace('[^0-9 -]', '', $this->request->gethtml('cc_number', 'post')); // strip non numerics from the text but leave spaces for the moment.
   ...
   */     
    ...
    private function paymentConfirmed() {
     ...
     private function getFormArray() {
        ...
        // Remove any spaces
        $cardNumber = ereg_replace('[ -]', '', $this->request->gethtml('cc_number', 'post'));
        $cardCVV = ereg_replace('[ -]', '', $this->request->gethtml('cc_cvv', 'post'));
