...
class Model_Review extends Model {
	...
	function insert_review($product_id){
		$sql = "insert into review set author = '?', customer_id = '?', product_id = '?', text = '?', rating = '?', date_added = now()";
      	$this->database->query($this->database->parse($sql, $this->customer->getFirstName() . ' ' . $this->customer->getLastName(), $this->customer->getId(), (int)$product_id, $this->request->sanitize('text', 'post'), $this->request->gethtml('rating', 'post')));
	}
	...
	function getRow_review($review_id){
		$result = $this->database->getRow("select r.review_id, r.author, r.rating, r.text, p.product_id, pd.name, p.price, p.tax_class_id, p.special_price, i.filename, r.date_added from review r left join product p on(r.product_id = p.product_id) left join product_description pd on(p.product_id = pd.product_id) left join image i on(p.image_id = i.image_id) where r.review_id = '" . (int)$review_id . "' and p.status = '1' and pd.language_id = '" . (int)$this->language->getId() . "' order by r.date_added desc");
		return $result;
	}
