...
<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
  ...
  if(RegularPrice > 0 && DiscountPercent >0){
    DiscountAmount = (((RegularPrice * (DiscountPercent/100))*100)/100).toFixed([2]);
  ...
  if(RegularPrice > 0 && DiscountAmount >0){
	...
	$('#discount_amount'+row).val(((DiscountAmount*100)/100).toFixed([2]));
  }  
  return;
  }
  ...
  if(Special_Price > 0 && Price >0){
    ...
    DiscountPrice = (((RegularPrice - (RegularPrice * (Percent/100)))*100)/100).toFixed([2]);
  ...
  if($('#discounts tr').size() >0){
	...
	$('#regular_price').val(((RegularPrice*100)/100).toFixed([2])); 
  ...
  if(Special_Price > 0 && RegularPrice >0){
	...
	$('#special_discount').val(Percent);
    DiscountPrice = (((Price - (Price * (Percent/100)))*100)/100).toFixed([2]);
	$('#special_price').val(DiscountPrice);
	$('#discount_special_price').val(DiscountPrice);    	
  }
  if($('#discounts tr').size() >0){
  ...
  $('#price').val(((Price*100)/100).toFixed([2])); 
  ...
  for (i=0; i < count; i++){
    ...
    var NewDiscountPrice = (((NewPrice * (DiscountPercent/100))*100)/100).toFixed([2]);
  ...
  if(Special_Price > 0 && RegularPrice >0){
    Percent = (100-((Special_Price/RegularPrice)*100)).toFixed([4]);
	$('#special_discount').val(Percent);
    $('#special_price').val(((Special_Price*100)/100).toFixed([2]));
    $('#discount_special_price').val(((Special_Price*100)/100).toFixed([2]));
  } else if(Special_Price == 0) {
    $('#special_price').val((0).toFixed([2])); 
    $('#special_discount').val((0).toFixed([4]));
	$('#discount_special_price').val((0).toFixed([2]));
  }
  return;
  }
  ...
  if(RegularPrice > 0 && SpecialDiscount >0){
    DiscountPrice = (((RegularPrice - (RegularPrice * (SpecialDiscount/100)))*100)/100).toFixed([2]);
	$('#special_price').val(DiscountPrice);
