...
// we first include the upload class, as we will need it here to deal with the uploaded file
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            // everything was fine !
            $mim = $handle->file_dst_name;
            $sql = "UPDATE `Movie_List` Set `Main_Dir` = '$mdir', `Movie_Title` = '$mtitle', `Release_Year` = '$ryear', `Genre` = '$genre', `About` = '$amov',  `Cast` = '$cast', `Directed_By` = '$dirby', `Date_Added` = '$dadd', `Have_Seen` = '$hseen', `Filename` = '$afile' , `Movie_Image` = '$mim' WHERE `Movie_Id` = '$mid'";
