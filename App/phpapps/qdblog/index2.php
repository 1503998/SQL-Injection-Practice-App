...
function entry_theme($input, $comment_count) 
{
	...
	$input = str_replace("[CONTENT]", "<a href=\"edit_entry.php?id={$value['id']}\">Edit</a>&nbsp;<a href=\"delete_entry.php?id={$value['id']}\">Delete</a><br/>" . $value['content'], $input);
...
function comment_theme($comment_theme, $comments, $entry_out)
{
	...
	global $value;
        ...
        while( $row = mysql_fetch_array($comments) ) {
                ...
                $tmp = str_replace("[CONTENT]", "<a href=\"del_comment.php?id={$row['id']}\">Delete</a><br/>"  . $row['content'], $tmp);
                $comment_work .= "\n" . $tmp;
        }
...
session_start();
include("qdblog.php");
if(user_allow("index.php", "admin", "mod")) {
global $conn;
global $theme;
theme();
...
echo "<br/><a href=\"create_entry.php\">New Entry</a><br/>";
...
while( $value = mysql_fetch_array($query)) {
	...
	global $value;
        ...
        //comment time
        if($_GET['comments'] == 'y') {
                $comments = mysql_query("SELECT * FROM $prefix"."comments WHERE entry_id = '" . $value['id'] . "' ORDER BY date;", $conn);
                $entry_out = comment_theme($comment_theme, $comments, $entry_out);
        } else { $entry_out = str_replace("[COMMENTS]", "", $entry_out); }
        echo $entry_out;
}
echo "<p><a href=\"admin.php\">Admin Page</a><br/><a href=\"logout.php\">Log Out</a></p>";
