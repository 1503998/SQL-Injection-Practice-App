...
class ControllerUser extends Controller { 
	...
	function initialize() {//$this->initialize();  Required in each function 
		if(!isset($this->dataLink)){
			...
			$this->modelAdminUser =& $this->model->get('model_admin_user');
		}
	}
  	function index() {
  	...
  	function getForm() {
    	...
    	if ($this->request->gethtml('user_id')) {
      		$view->set('update', $this->url->ssl('user', 'update', array('user_id' => $this->request->gethtml('user_id'))));
      		$view->set('delete', $this->url->ssl('user', 'delete', array('user_id' => $this->request->gethtml('user_id'),'user_validation' =>$this->session->get('user_validation'))));
    	}
