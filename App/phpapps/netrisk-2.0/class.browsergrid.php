...
class dataGrid
{
     ...
     *  @return void
     */
    function dataGrid($source, $formAction = "")
    {
        ...
        if (is_array($this->source)) {
            ...
            if ($this->totalRecords > 0) {
                ...
                if ($this->sortField != "") {
                    ...
                    array_multisort(${"zebraDataGrid_Status_Order"}, strtolower($this->sortDirection) == "asc" ? SORT_DESC : SORT_ASC,
        ...
        if (is_array($this->source) || @is_resource($this->sourceResult)) {
            ...
            if ($this->totalRecords > 0) {
                ...
                for ($i = 0; $i < count($this->recordsOnPageArray); $i++) {
                    ...
                    $options .= "<option value='" . $this->recordsOnPageArray[$i] . "' " . ($this->rowsOnPage == $this->recordsOnPageArray[$i] ? "selected=\"selected\"" : "") . ">" . $this->recordsOnPageArray[$i] . "</option>";
     ...
     *
     */
    function setDefaultSortColumn($columnName, $sortDirection = "DESC")
    {
    ...
    function setTemplate($template)
    {
        ...
        if (!isset($this->xtpl)) {
            ...
            $this->xtpl = new XTemplate($this->absolutePath . "/templates/" . $template . "/grid.xtpl");

        // if template object was already created
        } else {

            // restart it using the specified template
            $this->xtpl->restart($this->absolutePath . "/templates/" . $template . "/rankings.xtpl");
